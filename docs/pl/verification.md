...
| Windows 11 + Docker Desktop 29.3 + Git Bash | działa po trzech poprawkach znalezionych na tej maszynie: budowanie obrazu routera przy pierwszym uruchomieniu, `modelPicker` na koncie z polityką organizacji (obie w repo) i `gpus: all` na maszynie bez NVIDIA (teraz obsługiwane przez wykrywanie GPU) |
| wykrywanie platformy i GPU | `doctor` bez `.env`: `linux`, natywny silnik, GPU `yes`; `.env` z nakładką Windows i cudzym UID/`DOCKER_GID` poprawiony przy następnym uruchomieniu; `HARNESS_GPU=off` usuwa nakładkę, `auto` przywraca ją z pamięci podręcznej; `HARNESS_AUTODETECT=0` nie rusza `.env`; nieudane `--gpus all` (symulowane) → bez nakładki GPU; Git Bash (symulowany) → nakładka Windows i `/c/...` → `C:/...` |
